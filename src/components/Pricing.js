...
const PricingCard = ({ title, price, quota, children }) => (
...
const ButtonToDeveloperPortal = () => (
  <Link to={DEVELOPERS}>
    <Button color="primary">Sandbox</Button>
  </Link>
)
...
export const Pricing = ({ style, paid, free, isSignedIn }) => (
  <Row style={style} className="dark-text">
    <Col xs={12} md={6} style={paddingTop}>
      <PricingCard title="Free Tier" price="0" quota={free.quota}>
        <p>No credit card required</p>
        {isSignedIn ? <ButtonToDeveloperPortal /> : <ButtonToRegister />}
      </PricingCard>
    </Col>
    <Col xs={12} md={6} style={paddingTop}>
      <PricingCard title="Paid Tier" price="1" quota={paid.quota}>
        <p>Email support</p>
        {paid.isSubscribed ? (
